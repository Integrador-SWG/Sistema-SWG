    ...
    if (empty($status)) {
      ...
      $valid = false;
		...
		if ($valid) {
			...
			$sql = "UPDATE cliente set nombre = ?, apellido = ?, telefono = ?, email = ?, titulo =?, user = ?, pass = ?, status = ? WHERE idcliente = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($nombre,$apellido,$telefono,$email,$titulo,$user,$pass,$status,$idcliente));
			Database::disconnect();
